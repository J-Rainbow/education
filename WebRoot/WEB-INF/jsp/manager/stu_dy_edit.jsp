	...
	$().ready(function () {
		$('#sub').click(function(){
			if($('#stitle').val() == "" || $('#scontext').val() == "" ){
				...
				fm.action = "${pageContext.request.contextPath }/rest/sdynamics/updateSdy";
			}
		});
    });
</script>
		...
		<div class="sty_dy_content">
			...
			<form action="" id="fm" method="post">
				<c:forEach var = "sdy" items="${sdy }">
					<input type="hidden" name="sid" value="${sdy.sid }">
					<input type="hidden" name="speople" value="${userName }">
					<div class="cen">
						<span>新闻标题:</span>
						<input id="stitle" name="stitle" value="${sdy.stitle }" class="sty_dy_title" placeholder="请输入新闻标题">
					</div>
					<div class="cen">
						<span>新闻内容:</span>
						<textarea id="scontext" name="scontext" class="sty_dy_msg" placeholder="请输入新闻内容">${sdy.scontext }</textarea>
					</div>
				</c:forEach>
				<div class="clear"></div>
